...
对于一个图，将其所有节点均划分到两个集合。若任意两个**相邻节点**均在不同集合，则称该图是二分图。

对于不联通的图，若每一块联通部分均为二分图，则该图是二分图。

判断一个图是否是二分图，常用DFS/BFS染色法，以任意一点为起始点，将其标记为颜色 $A$ 或 $B$，然后从该点开始不断扩展，将每个节点的相邻节点染成另一种颜色。
