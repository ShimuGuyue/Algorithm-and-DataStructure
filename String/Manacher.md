...
当 $i$ 在 $P$ 的右侧时，最右回文串无法为求解当前位置回文半径提供信息，因此需要将当前位置**更新**为新的最右回文串。从当前位置开始向两侧暴力地进行拓展，求得回文半径 $len_i$，由于此时以 $i$ 为中心的回文串右端点一定在 $r$ 的右侧，将最右回文串更新为 $p = i,\ l = i - len_i + 1,\ r = i + len_i - 1$。
