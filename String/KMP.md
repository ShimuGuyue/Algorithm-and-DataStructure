...
* 一般地，border 通常指字符串地最大 border，具体根据预警判断。
...
对于模式串的匹配指针 $i$，正在匹配第 $i$ 个位置同样也可以表示已经匹配了 $i$ 个字符。而对于一个长度为 $n$ 的模式串，统计计数共计有 $n + 1$ 种不同情况。当需要匹配所有出现的字符串时，应将 nexts 数组的第一维长度设为 $n + 1$，用 `nexts[n][ch]` 表示已经匹配成功一个模式串之后再匹配下一个模式串应该跳转到哪个位置，以便匹配所有模式串的位置。~~或者也可以不额外开一个位置，匹配完成时根据 fail 链回退亦可。~~
