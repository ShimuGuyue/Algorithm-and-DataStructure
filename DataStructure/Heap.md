...
堆是一种用于维护**可比较**元素的**优先顺序**（较大值优先、较小值优先等，以下内容以*较小值优先*为例）的数据结构。**堆顶元素**的优先级最高，且仅对外开放取堆顶操作访问元素。

---

堆有多种实现方式，一般情况下为d叉堆中的**二叉堆**。

d叉堆的实现是一颗**完全d叉树**，由于其每层的节点数为以 $a_1 = 1,\ q = d$ 的等比数列，将每一层节点按顺序编号，任意节点则的子节点编号极其父节点编号可通过计算得出，因此可以使用顺序表方便地进行存储。这种存储方式又称为**堆式存储**。
...
插入操作的时间复杂度为树高，即 $O(\log_d n)$。$d$ 越大时复杂度越优，为 $1$ 时退化为链表。
...
删除堆顶元素时，也要保持完全d叉树结构。用最后一个节点的值覆盖将要被删除的根节点的值，然后删除最后一个节点实现删除操作。

删除完成后，根节点处的优先级顺序可能并不正确，需要进行修正。

修正过程使用**下沉**操作，即当待修正节点的子节点中存在某个节点的优先级更高时，将二者位置交换。应从所有子节点中选择优先级最高的节点与其交换，以保证交换之后的优先级正确。

删除操作的时间复杂度为树高乘以每个节点的子节点数量，即 $O(d\ log_d n)$，当 $d$ 在 $e$ 附近时复杂度最优，所以一般取 $2$ 或 $3$。
...
public:
    ...
    {
        const int node{ static_cast(heap_.size()) };
